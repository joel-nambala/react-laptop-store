const laptopData = [
  {
    ...
    image: "./img/huawei-matebook-pro.jpg",
  ...
  },
  {
    ...
    image: "./img/asus-zenbook-flip-s.jpg",
    cpu: "Intel Core i7, 8th generation",
    ram: "16GB",
    storage: "512 GB SSD",
  ...
  },
  {
    ...
    image: "./img/samsung-notebook-9.jpg",
    cpu: "Intel Core i7, 8th generation",
    ram: "16GB",
    storage: "256 GB SSD",
  ...
  return (
    <div className="cards-container">
      {laptopData.map((laptop) => (
        <Laptop laptopObj={laptop} key={laptop.id} />
      ))}
    </div>
  );
};

const Laptop = function ({ laptopObj }) {
  return (
    <figure className="card">
      <div className="card__hero">
        <img
          src={laptopObj.image}
          alt={laptopObj.productName}
          className="card__img"
        />
      </div>
      <h2 className="card__name">{laptopObj.productName}</h2>
      <p className="card__detail">
        <span className="emoji-left">🖥</span> {laptopObj.screen}
      </p>
      <p className="card__detail">
        <span className="emoji-left">🧮</span> {laptopObj.cpu}
      </p>
      <div className="card__footer">
        <p className="card__price">${laptopObj.price}</p>
        <a href="laptop.html" className="card__link">
          Check it out <span className="emoji-right">👉</span>
        </a>
      </div>
    </figure>
  );
